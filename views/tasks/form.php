<form method="post" action="?controller=tasks&action=create">
  ...
  <div class="form-group">
    <label for="name" class="trn"><input type="checkbox" class="form-control" name="active" id="active" value="<?php echo $task->active; ?>" <?php if($task->active == 1){ echo "checked"; } ?>>Active</label>
  </div>
  <?php } ?>
  <div class="form-group">
